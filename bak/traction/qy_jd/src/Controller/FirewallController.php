...
class FirewallController extends ControllerBase {
  ...
  public function listenWall($route_id) {
    ...
    //线路对应防火墙
    $netcoms = $db_service->load_netcom(array('type' => $route_id));
    $wall_route = array();
    foreach($netcoms as $netcom) {
      if(array_key_exists($netcom->type, $wall_route)) {
        $wall_route[$netcom->type]['hosts'][$netcom->id] = array(
          'ip' => $netcom->ip,
          'user' => $netcom->username,
          'pass' => $netcom->password
        );
      } else {
        $wall_route[$netcom->type]['route'] = $route;
        $wall_route[$netcom->type]['hosts'][$netcom->id] = array(
          'ip' => $netcom->ip,
          'user' => $netcom->username,
          'pass' => $netcom->password
      ...
      foreach($hosts as $key => $host) {
        if(!isset($host['cookie'])) {
          ...
          if(!empty($cookies)) {
            ...
            $wall_route[$route_id]['hosts'][$key]['cookie'] = $cookies[$key];
          }
        }
        if(isset($host['cookie'])) {
          $visit->check_host($db_service, $item['route'], $host, $ln, $temp_qy_ips);
        }
        //重新请求了就关闭原来的循环
  ...
  public function listenkill() {
    ...
    $traction_list = FactoryAjaxListBuill::getList('qy_jd', 'traction');
    $traction_list_path = $route_value_path . '/traction_list.html';
    $traction_filter = FactoryAjaxListBuill::getList('qy_jd', 'tractionfilter');
    $traction_filter_path = $route_value_path . '/traction_filter.html';
    $flow_monitor = FactoryAjaxListBuill::getList('qy_jd', 'monitor');
    ...
    while(true) {
      ...
      $flow_monitor_html = $flow_monitor->createHtml($alarms, $routes, $qy_ips);
