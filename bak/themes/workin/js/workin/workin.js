(function ($, Drupal) {
  ...
  //pageSetUp();
  $('ol.breadcrumb li:first').addClass('fa fa-home');
  $('ol.breadcrumb li').addClass('fa');
  $('ol.breadcrumb li:last').removeClass('fa');
 ...
 */
   ...
   */
	initApp = (function(app) {
        ...
        $.SmartMessageBox({
          ...
          content : $this.data('logout-msg') || "退出后关闭当前浏览器会提升账号的安全性。",
          buttons : '[否][是]'
      ...
      resetWidgets: function($this){
        $.SmartMessageBox({
          title : "<i class='fa fa-refresh' style='color:green'></i> 清空本地缓存",
          content : $this.data('reset-msg') || "确认重置控件状态并清空本地缓存吗?1",
          buttons : '[否][是]'
        }, function(ButtonPressed) {
	...
	/* ~ END: SMART ACTIONS */
		...
		app.domReadyMisc = function() {
			...
			var d = $("#activity > .badge");
			if (parseInt(d.text()) > 0) {
				d.addClass("bg-color-red bounceIn animated");
				//clear memory reference
				d = null;
			}
		};
   ...
   */
    ...
    /*
        ...
        var win = $(window);
        console.log(win);
        timeout_id = win[str_setTimeout](function () {
          elems.each(function () {
            var width;
            var height;
            var elem = $(this),
