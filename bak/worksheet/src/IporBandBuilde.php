 ...
 * @file
 * Contains file
 * \Drupal\worksheet\IporBandBuilde.
 */
...
class IporBandBuilde {
  ...
  protected function buildRow() {
    ...
    $entity_type = 'work_sheet_ip';
    $type='130,131,132,133';
    ...
    if(!empty($data)){
      ...
      array_push($rows,array('','合计',$num,$ye_num,$work,round(($spend_time/60)/empty($num)?1:$num,1),round($spend_time/60,1)));
      array_push($rows,array('工单占比','白天',round(($all_day-$ye_num)/empty($all_day)?1:$all_day*100, 2)."%",'夜晚',round($ye_num/empty($all_day)?1:$all_day *100, 2)."%"));
    }
    return $rows;
  }
