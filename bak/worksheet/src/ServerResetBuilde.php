 ...
 * @file
 * Contains file
 * \Drupal\worksheet\ServerResetBuilde
 */
...
class ServerResetBuilde {
  ...
  protected function buildRow() {
    ...
    if(!empty($data)){
      ...
      array_push($rows,array('工单占比','白天',round(($all_day-$ye_num)/$all_day*100, 2)."%",'夜晚',round($ye_num/$all_day*100, 2)."%",'','系统占比例',round($win03/$system_num*100, 2)."%",round($win08/$system_num*100, 2)."%",round($win2012/$system_num*100, 2)."%",round($centos/$system_num*100, 2)."%",round($ubuntu/$system_num*100, 2)."%",round($system/$system_num*100, 2)."%",round($uninstall/$system_num*100, 2)."%"));
    }else{
      $rows=null;
    }
    return $rows;
