 ...
 */
class PayproAuditHistoryListBuilder {
  ...
  private function getPersonalAudit($auid) {
    $query = db_select('paypro__aids', 'ra');
  ...
  public function render() {
    ...
    $build['tips'] = ['#markup' => SafeMarkup::format('友情提醒: 前面几列的链接，可打开新窗口浏览，本页仅列出当前用户已审批过的需求数据! <br/><font color=red>审批意见列</font>仅列出第一条审批意见,如需更多，进入详情页面查看！', [])];
    return $build;
