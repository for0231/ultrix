(function($, Drupal, drupalSettings) {
  "use strict";
  Drupal.behaviors.paypro_detail = {
    attach: function (context) {
      $("#payprodetails").jqGrid({
        url: Drupal.url('ajax/paypro/'+ drupalSettings.paypro.id +'/details'),
        datatype: "json",
			  height : 'auto',
        colNames: ['ID', '付款单编号', '付款单名称', '合同号', '收款账号', '部门', '建单人', '币种',  '待付金额', '总金额',  '工单状态', '审批状态', '创建时间'],
        colModel:[
        ...
        subGridRowExpanded: function(subgrid_id, row_id) {
          ...
          $("#"+subgrid_table_id).jqGrid({
            url: Drupal.url('ajax/purchase/statistic/collection'+"?id="+row_id),
            datatype: "json",
            ...
            colModel: [
              ...
              subGridRowExpanded: function(subgrid_id, row_id) {
                ...
                $("#"+subgrid_table_id).jqGrid({
                  ...
                  datatype: "json",
                  colNames: ['ID', '配件名称', '类型', '部门', '建单人', '币种', '金额', '单价', '数量', '需求单', '需求单状态', '需求单审批状态', '创建时间'],
                  colModel: [
                    {name: 'id', index: 'id', width: 40, editable: false},
                    {name: 'no', index: 'no', width: 200, editable: false},
      ...
      $("#check_check_accept").once().click(function() {
        ...
        // detail
				$("#payprodetails").jqGrid('setGridWidth', $("#content").width());

        if ($(window).width() <= 460) {
          $("#payprodetails").jqGrid('hideCol', ['id', 'depart', 'uid', 'status', 'audit', 'created']);
        } else {
          $("#payprodetails").jqGrid('showCol', ['id', 'depart', 'uid', 'status', 'audit', 'created']);
        }
			})
      // remove classes
