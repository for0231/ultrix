(function($, Drupal, drupalSettings) {
  ...
  Drupal.behaviors.paypres = {
    attach: function (context) {
      ...
      $("#statis").jqGrid({
        url: Drupal.url('ajax/paypro/statistic/collection'),
        datatype: "json",
			  height : 'auto',
        colNames: ['ID', '编号', '部门', '建单人', '币种', '金额', '工单状态', '审批状态', '创建时间'],
        colModel:[
        ...
        subGridRowExpanded: function(subgrid_id, row_id) {
          ...
          $("#"+subgrid_table_id).jqGrid({
            url: Drupal.url('ajax/paypro/paypre/statistic/collection'+"?id="+row_id),
            datatype: "json",
            colNames: ['ID', '付款单编号', '部门', '建单人', '币种', '金额', '工单状态', '审批状态', '创建时间'],
            colModel: [
              {name: 'id', index: 'id', width: 40, editable: false},
              ...
              subGridRowExpanded: function(subgrid_id, row_id) {
                ...
                $("#"+subgrid_table_id).jqGrid({
                  ...
                  datatype: "json",
                  colNames: ['ID', '采购单编号', '部门', '建单人', '金额', '工单状态', '审批状态', '创建时间'],
                  colModel: [
                    {name: 'id', index: 'id', width: 40, editable: false},
                    {name: 'no', index: 'no', width: 200, editable: false},
                    {name: 'depart', index: 'depart', width: 40, editable: false},
      ...
      $("#statis").jqGrid('navGrid', "#statisnav", {
        edit : false,
        add : false,
        del : false,
        search: false,
