...
class AuditService {
  ...
  /**
   * @description 获取被审批的数据模型.
   * @param $audit_id
   * @return type, title string
   */
  public function getAuditModelData($audit_id) {
    $data = $this->getDatafromDbforAuditData($audit_id);
    $type = [];
    $title = '';
    foreach ($data as $row) {
      if (!empty($row['bundle'])) {
        $function = $row['bundle'] . '_load';
        $entity = call_user_func_array($function, [$row['entity_id']]);
        $type[] = \Drupal::entityManager()->getStorage($row['bundle'])->getEntityType()->getLabel()->getUntranslatedString();
        $title .= $entity->get('title')->value . '<br/>';
      }
    }
    return [reset($type), $title];
  }

  /**
   * @description 迭代获取数据.
   */
  private function getIterateData($query, $data, $i) {
    foreach ($query as $row) {
      $data[$i]['bundle'] = @$row->bundle;
      $data[$i]['entity_id'] = @$row->entity_id;
      $i++;
    }
    return [$data, $i];
  }
   ...
   */
  private function getDatafromDbforAuditData($audit_id) {
    $data = [];
    $i = 0;
    $requirement_query = $this->database->select('requirement__aids', 'ra')
      ->fields('ra', ['bundle', 'entity_id'])
      ->condition('aids_target_id', $audit_id)
      ->execute()
      ->fetchAll();

    $purchase_query = $this->database->select('purchase__aids', 'ca')
      ->fields('ca', ['bundle', 'entity_id'])
      ->condition('aids_target_id', $audit_id)
      ->execute()
      ->fetchAll();

    $paypre_query = $this->database->select('paypre__aids', 'ya')
      ->fields('ya', ['bundle', 'entity_id'])
      ->condition('aids_target_id', $audit_id)
      ->execute()
      ->fetchAll();

    $paypro_query = $this->database->select('paypro__aids', 'pa')
      ->fields('pa', ['bundle', 'entity_id'])
      ->condition('aids_target_id', $audit_id)
      ->execute()
      ->fetchAll();

    list($da, $i) = $this->getIterateData($requirement_query, $data, $i);
    list($da, $i) = $this->getIterateData($purchase_query, $da, $i);
    list($da, $i) = $this->getIterateData($paypre_query, $da, $i);
    list($da, $i) = $this->getIterateData($paypro_query, $da, $i);
