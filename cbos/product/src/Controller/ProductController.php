...
class ProductController extends ControllerBase implements ContainerInjectionInterface {
   ...
   *
   * @param \Drupal\product\Entity\ProductInterface $product
   *   A Product  object.
   *
   * @return array
  ...
  public function revisionOverview(ProductInterface $product) {
    ...
    $revert_permission = (($account->hasPermission("revert all product revisions") || $account->hasPermission('administer products')));
    $delete_permission = (($account->hasPermission("delete all product revisions") || $account->hasPermission('administer products')));
    ...
    foreach (array_reverse($vids) as $vid) {
      /** @var \Drupal\product\ProductInterface $revision */
